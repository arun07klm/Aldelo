...
{
    public class TotalPayoutController : ApiController
    {
        ...
        public List<MenuItem> GetTotalRefundAmount(DataTable dtBody)
        {
            ...
            try
            {
                ...
                {
                    query = @" SELECT EmployeeFiles.FirstName AS Name, SUM(PayOuts.PayOutAmount) AS Amount
                            FROM     (EmployeeFiles INNER JOIN
                         PayOuts ON EmployeeFiles.EmployeeID = PayOuts.NonCashierEmployeeID OR EmployeeFiles.EmployeeID = PayOuts.WageAdvanceEmployeeID)
                    GROUP BY EmployeeFiles.FirstName";
                }
                else
                {
                    query = @" SELECT EmployeeFiles.FirstName AS Name, SUM(PayOuts.PayOutAmount) AS Amount
                        FROM     (EmployeeFiles INNER JOIN
                  PayOuts ON EmployeeFiles.EmployeeID = PayOuts.NonCashierEmployeeID OR EmployeeFiles.EmployeeID = PayOuts.WageAdvanceEmployeeID)
                    where format(PayOutDateTime,'dd-mm-yyyy') between" + dtBody.Rows[0]["fromDate"].ToString() + "@ and " + dtBody.Rows[0]["toDate"].ToString()+"@ GROUP BY EmployeeFiles.FirstName";
                }
